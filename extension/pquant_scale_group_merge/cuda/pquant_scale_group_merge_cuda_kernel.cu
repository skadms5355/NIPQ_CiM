...
template <typename scalar_t>
__global__ void pquant_scale_merge_cuda_forward_kernel(
    ...
    for ( int g = 0; g < groups; g++ ) {
        ...
        // Access scaled_steps using the index g
        float scaled_step = scaled_steps[g];
        // Print scaled_step for each thread
        // if (threadIdx.x == 0 && blockIdx.x == 0) {
        // printf("g %d, scaled_step: %f\n", g, scaled_step);} 
        // printf("[col: %d, idx: %d] g: %d, step: %f, input: %f\n", col, idx, g, scaled_steps[g], input[g][idx]);
...
Tensor& pquant_scale_merge_cuda(
    Tensor&                         output,
    ...
    // get GPU storage for scaled_steps
    auto d_scaled_steps_storage = empty({groups}, output.options().dtype(/*'at::'*/kFloat));
    auto d_scaled_steps = static_cast<float*>(d_scaled_steps_storage.data_ptr());
    
    // copy scaled_steps information from CPU to GPU
    native::copy_(d_scaled_steps_storage, from_blob(scaled_steps, {groups}, at::dtype(at::kFloat).device(at::kCPU).memory_format(at::MemoryFormat::Contiguous)));

    // launch kernel
    const dim3 grid( CEIL(num_features_per_batch, THREADS), batch_size);
    const dim3 blocks( THREADS, 1, 1 );
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(output.scalar_type(), "pquant_scale_merge_forward_cuda", ([&] {
        pquant_scale_merge_cuda_forward_kernel<scalar_t><<<grid, blocks>>>(
        //pquant_scale_merge_cuda_forward_kernel<scalar_t><<<grid, blocks, 0, stream.stream()>>>(
...
Tensor pquant_scale_group_merge_cuda_forward(
    ...
    if ( pbits == 32 ) {
        ...
        std::vector<float> scaled_steps_per_group(groups, weight);
        for (int g = 0; g < groups; g++) {
            scaled_steps_per_group[g] *= step[g];
        }
        AT_DISPATCH_FLOATING_TYPES_AND_HALF(output.scalar_type(), "pquant_scale_merge_cuda", ([&] {
            pquant_scale_merge_cuda<scalar_t>(output, input, scaled_steps_per_group.data(),
