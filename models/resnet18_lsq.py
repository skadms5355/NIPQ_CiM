...
__all__ = ['lsq_resnet18']

def conv3x3(in_planes, out_planes, wbits, stride=1):
    """3x3 convolution with padding"""
    return QConv(in_planes, out_planes, wbits=wbits, kernel_size=3, stride=stride, padding=1,
                      groups=1, bias=False)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, **kwargs):
        ...
        self.act1 = add_act(abits=self.abits)
        self.conv1 = conv3x3(inplanes, planes, self.wbits, stride=stride)
        self.bn1 = nn.BatchNorm2d(planes)
        self.act2 = add_act(abits=self.abits)
        self.conv2 = conv3x3(planes, planes, self.wbits, stride=1)
