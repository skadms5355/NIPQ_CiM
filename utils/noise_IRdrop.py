...
class IrSolver(object):
    ...
    def caliout(self) -> "output current w.r.t. IR drop":
        ...
        """
        # start1 = time.time()
        # start current value (I = G(input each row) * V)
        if self.device == "cpu":
        ...
        if self.device == "cpu":
            ...
            node_i = torch.LongTensor([self.mat_row, self.mat_col]).cuda() #flatten matrix
        node_v = torch.stack(self.mat_data)
        import pdb; pdb.set_trace()
        # current_mat = current_mat.to('cpu')
    ...
    def _nodematgen(self):
        ...
        electrode = ['top', 'bot'] # WL line plate and BL plate
        counter = 0
...
if __name__ == '__main__':
    crxb_size = 128
    vdd = 1 # due to vt drop (set operation range -0.8V ~ -1.4V)
    Gwire = 1 # wire conductance S
    Gload = 10 # ADC and DAC loading conductance
    Gmin = 1/3e6
    Gmax = 1/3e3
    x = torch.rand(crxb_size, 1, 1, 1, 1)*vdd # generating input
    Gmat = torch.rand(crxb_size, crxb_size, 1, 1)*(Gmax-Gmin)+Gmin # generating crxb
    iout_ideal = torch.matmul(Gmat.unsqueeze(4).permute(2, 3, 4, 1, 0), x.permute(2, 3, 4, 0 ,1)).cuda()# ideal current output
    crxb = IrSolver(Rsize=crxb_size, Csize=crxb_size, Gwire=Gwire, Gload=Gload, input_x=x, Gmat=Gmat)
    crxb.resetcoo()
    output_crxb = crxb.caliout()
    print(((iout_ideal - output_crxb)/iout_ideal*100).abs().max())
    print(output_crxb/iout_ideal)
    import pdb; pdb.set_trace()